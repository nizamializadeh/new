                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <div class="product-payment-inner-st">
                            <ul style="    padding: 0px 0px 20px 0px;background: #fff; font-weight: bold;font-size: 20px;">
                                @foreach(languages() as $language)
                                    <li class="col-md-2"><a  href="{{route('branchplace',['id' => $id,'lang_id' => $language->id])}}">{{$language->name}}</a></li>
                                @endforeach
                            </ul>
                        </div>
                        <div class="product-status-wrap drp-lst">
                            ...
                            <div class="add-product">
                                <a href="{{route('placebranch.create',['id' => $id])}}">Add Branch</a>
                            </div>
                            ...
                            <thead>
                                ...
                                @foreach($branches as $branch)
                                    ...
                                    <tr class = "item ">
                                        <td>{{$branch->id}}</td>
                                        <td>{{$branch->name}}</td>
                                        <td><input id="toggle-demo" data-table="properties" data-row="{{$branch->id}}"  type="checkbox" {{($branch->isVisible) ? 'checked' : ""}} class="statusCheckBox"  data-toggle="toggle"></td>
                                        <td>
